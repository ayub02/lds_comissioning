...
def decode_time(_buffer_len, _buffer):
    ...
    return datetime(int(year), int(month), int(day), int(hour)+hour_offset, int(minute), int(second), int(millisecond) * 1000)
...
T = Text(root, height=25, width=180)
