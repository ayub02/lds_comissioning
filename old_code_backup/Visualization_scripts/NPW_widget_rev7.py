...
def get_data(_tag):
    if option_var.get() == 1:
        _query = sql.SQL("SELECT * FROM mfmdata6dec where itemid= %s ORDER BY itemtimestamp DESC LIMIT %s;")
    ...
    if option_var.get() == 2:
        ...
        _query = sql.SQL("SELECT * FROM mfmdata6dec WHERE itemid= %s AND itemtimestamp BETWEEN %s AND %s ORDER BY itemtimestamp DESC;")
...
while True:
    try:
        ...
        conn = psycopg2.connect(dbname='mfmdata10Dec', user='postgres', password='@intech#123', host='10.1.17.149',
                                port='5432')
