...
def new_update_listbox(_lbx, _log, _tag):
    ...
    if _opcTag:
        ...
        _lbx.df = pd.DataFrame(_dfData, columns=['OPC_Tag', 'Buffer', 'OPC_timestamp', 'Pressure_values',
                                                 'Buffer_timestamp'])
        for val in _lbx.df['Buffer_timestamp']:
            _lbx.insert(END, val.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3])


def update_listbox():
    new_update_listbox(lbx1, log1, 'MKT.NPW.ByteArray')
    new_update_listbox(lbx2, log2, 'MOV60.NPW.ByteArray')
    new_update_listbox(lbx3, log3, 'BV61.NPW.ByteArray')
    new_update_listbox(lbx4, log4, 'BV62.NPW.ByteArray')
    new_update_listbox(lbx5, log5, 'BV63.NPW.ByteArray')
    new_update_listbox(lbx6, log6, 'BV64.NPW.ByteArray')
    new_update_listbox(lbx7, log7, 'BV65.NPW.ByteArray')
    new_update_listbox(lbx8, log8, 'BV66.NPW.ByteArray')
    new_update_listbox(lbx9, log9, 'KBS1.NPW.ByteArray')
...
def newPlot(_overlay, _export=False):
    ...
    if _export:
        ...
        _tag = _lbx.df['OPC_Tag'][_lbx.df['Buffer_timestamp'] == _pd_Timestamp].tolist()[0]
        _rawBuffer = _lbx.df['Buffer'][_lbx.df['Buffer_timestamp'] == _pd_Timestamp].tolist()[0]
        _opcTimestamp = _lbx.df['OPC_timestamp'][_lbx.df['Buffer_timestamp'] == _pd_Timestamp].tolist()[0]
        _pVal = _lbx.df['Pressure_values'][_lbx.df['Buffer_timestamp'] == _pd_Timestamp].tolist()[0]
        _bufferTimestamp = _lbx.df['Buffer_timestamp'][_lbx.df['Buffer_timestamp'] == _pd_Timestamp].tolist()[0]
        ...
        _pd_Timestamp = pd.Timestamp(datetime.strptime(_lbx.get(_idx), '%Y-%m-%d %H:%M:%S.%f'))
        func_plot(_lbx.df['OPC_Tag'][_lbx.df['Buffer_timestamp'] == _pd_Timestamp].tolist()[0],
                  _lbx.df['Buffer'][_lbx.df['Buffer_timestamp'] == _pd_Timestamp].tolist()[0],
                  _lbx.df['OPC_timestamp'][_lbx.df['Buffer_timestamp'] == _pd_Timestamp].tolist()[0],
                  _lbx.df['Pressure_values'][_lbx.df['Buffer_timestamp'] == _pd_Timestamp].tolist()[0],
                  _lbx.df['Buffer_timestamp'][_lbx.df['Buffer_timestamp'] == _pd_Timestamp].tolist()[0],
...
def func_plot(_OPC_Tag, _buffer, _OPC_timestamp, _pVal, _buffer_timestamp, _on_existing=False):
    ...
    if _buffer_timestamp == 'Invalid Timestamp':
        plt.plot(_pVal_to_plot, linestyle=_linestyle, color=_color, label=_OPC_Tag + '     ' + 'OPC TS:' + _OPC_timestamp.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
...
MKTtoKBS = Radiobutton(root, text="MKT ---> KBS", variable=option_var, value=1).grid(row=6, column=7, sticky='w')
KBStoMKT = Radiobutton(root, text="MKT <--- KBS", variable=option_var, value=2).grid(row=7, column=7, sticky='w')
...
label_MKT = Label(root, text="MKT", justify=CENTER).grid(row=0, column=0)
label_MOV60 = Label(root, text="MOV60", justify=CENTER).grid(row=0, column=1)
label_BV61 = Label(root, text="BV61", justify=CENTER).grid(row=0, column=2)
label_BV62 = Label(root, text="BV62", justify=CENTER).grid(row=0, column=3)
label_BV63 = Label(root, text="BV63", justify=CENTER).grid(row=0, column=4)
label_BV64 = Label(root, text="BV64", justify=CENTER).grid(row=0, column=5)
label_BV65 = Label(root, text="BV65", justify=CENTER).grid(row=0, column=6)
label_BV66 = Label(root, text="BV66", justify=CENTER).grid(row=0, column=7)
label_KBS = Label(root, text="KBS", justify=CENTER).grid(row=0, column=8)
