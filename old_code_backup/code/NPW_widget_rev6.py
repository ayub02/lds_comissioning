...
def newPlot(_overlay, _export=False):
    ...
    if _export:
        ...
        _filename = _tag + '__' + 'opc_timestamp' + '__' + _opcTimestamp.strftime('%Y_%m_%d__%H_%M_%S') + '.csv'
        _filename = (_tag + '__' + 'opc_timestamp' + '__' + _opcTimestamp.strftime('%Y_%m_%d__%H_%M_%S')).replace('.', '_')
...
now = datetime.now()
cal_from = Calendar(root, selectmode="day", year=now.year, month=now.month, day=now.day).grid(row=8, column=0, columnspan=2)
cal_to = Calendar(root, selectmode="day", year=now.year, month=now.month, day=now.day).grid(row=8, column=3, columnspan=3)

T = Text(root, height=25, width=190).grid(row=10, columnspan=15, sticky='w')
