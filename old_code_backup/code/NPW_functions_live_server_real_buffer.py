...
cases = pd.read_csv('../data/prepoc2_buffers/Test11.csv')
...
for i, topic in enumerate(topics):
    ...
    buff_map[topic] = byteArrayJson
    topic_times[topic] = now_time + timedelta(seconds=rel_times[topic]+1)
    topic_sent[topic] = False


count = 0
print('Schedule')
for i, topic in enumerate(topics):
    print(topic, '\t', topic_times[topic].strftime('%Y-%m-%d %H:%M:%S.%f'))

while count < len(topics):
    for i, topic in enumerate(topics):
        if datetime.now() > topic_times[topic] and topic_sent[topic] is False:
            print('Printing topic ', topic, ' after delay of ', round((datetime.now()-topic_times[topic]).total_seconds(), 3), ' sec')
            topic_sent[topic] = True
            publislhBuffer(buff_map[topic], topic_map[topic])
            count += 1
