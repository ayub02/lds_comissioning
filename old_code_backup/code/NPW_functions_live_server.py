...
for i, topic in enumerate(topics):
    ...
    topic_times[topic] = now_time + timedelta(seconds=rel_times[topic]+10)
...
while count < len(topics):
    for i, topic in enumerate(topics):
        if datetime.now() > topic_times[topic] and topic_sent[topic] is False:
            print('Printing topic ', topic, ' after delay of ', round((datetime.now()-topic_times[topic]).total_seconds(), 2), ' sec')
